...
namespace ScrollToInSolutionExplorer
{
    ...
    public static class SolutionExplorerHelpers
    {
        ...
        private static async Task SelectUIHItemAsync(AsyncPackage package, List<string> nodeTreeNames)
        {
            ...
            foreach (var part in nodeTreeNames)
            {
        ...
        )
        {
            ...
            else
            {
                ...
                hierarchy.GetCanonicalName(itemid, out var pstrCononicalName);
                hierarchy.GetProperty(itemid, (int)__VSHPROPID.VSHPROPID_Name, out var pDisplayed);
                Debug.WriteLine($"INFO: Walking hierarchy node: {pstrCononicalName} = {pDisplayed}");
                nodeNamesFound.Add((string)pDisplayed);

                if (nodeName.Length > 0)
                {
                    //If we find match, terminate node enumerating
                    if (string.Equals(nodeName, nodeNamesFound.Last(), StringComparison.InvariantCulture))
                    {
                ...
                ++recursionLevel;
                var siblingId = IsVisibleOnlySearch || (IsHeirSolutionRoot && recursionLevel == 1)
                    ? (int)__VSHPROPID.VSHPROPID_FirstVisibleChild
                    : (int)__VSHPROPID.VSHPROPID_FirstChild;

                hResult = hierarchy.GetProperty(itemid, siblingId, out pDisplayed);
                ...
                if (hResult == VSConstants.S_OK)
                {
                    ...
                    uint childId = GetItemId(pDisplayed);
                    while (childId != VSConstants.VSITEMID_NIL)
                    {
                        ...
                        siblingId = IsVisibleOnlySearch || (IsHeirSolutionRoot && recursionLevel == 1)
                            ? (int)__VSHPROPID.VSHPROPID_NextVisibleSibling
                            : (int)__VSHPROPID.VSHPROPID_NextSibling;

                        hResult = hierarchy.GetProperty(childId, siblingId, out pDisplayed);
                        if (VSConstants.S_OK == hResult)
                        {
                            childId = GetItemId(pDisplayed);
                        }
                        else
                        {
                            ErrorHandler.ThrowOnFailure(hResult);
